...
export default function ErpSidebar({
  ...
  const displayUserName = userName ?? user?.email ?? 'Admin User'
  const displayRole = userRole ?? String(user?.role ?? 'System Administrator')
  const displayInitials = userInitials ?? displayUserName.slice(0, 2).toUpperCase()
      ...
      {displayUserName && (
        <div className="border-t border-slate-200 px-4 py-3">
          <button 
            ...
            className="flex w-full items-center gap-3 text-left transition-colors hover:bg-slate-100 p-2 rounded-lg -mx-2"
          >
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-slate-800">{displayUserName}</p>
              <p className="text-[10px] font-medium uppercase tracking-wider text-slate-400">
                {displayRole}
              </p>
            </div>
